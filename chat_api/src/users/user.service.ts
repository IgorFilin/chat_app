...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto, userIP: string) {
    try {
      ...
      if (findUser) {
        ...
        const imagePath = path.join('https://filin.tech/api', 'static', 'image', 'default_photo_user.webp');
  ...
  async getPhoto(authToken: string) {
    if (authToken) {
      ...
      if (user?.authToken) {
        ...
        const dirname = process.cwd();
        const searchPath = path.join('https://filin.tech/api', 'static', 'image', oldAvatarName);
  ...
  async setPhoto(userId: string, newAvatar: any) {
    try {
      ...
      const savePath = path.join('https://filin.tech/api', 'static', 'image', newAvatar.avatar.originalName);
